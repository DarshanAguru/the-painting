...
The gameplay is very simple, the user has to sign-up and log in after which the game and scoring process will be described. They will then start and the user is expected to give correct answers as scores will be awarded as per the scoring criteria.
...
There are dead-ends in the project that will arise when users fail to get the correct logic required for solving the question.
...
- The user data insights have a table that has records for the time and score of each question that the user has attempted.
- The user data insights also have two graphs plotted, one shows the question and score for each question (line plot) and the other is the pie chart that shows what part of communication skills and problem-solving skills account for the total score.

## Scoring

The scoring logic is very subtle, It is based on the time the user takes to solve the question. There is no negative scoring for the wrong answer, scoring is based on the time taken.
...
The third way is where the user gets trapped in the second trap and reaches a dead end. The second trap is set in the 6th question.
...
It is necessary to check if your requirements are already installed, please refer to the requirements.txt file in the repository.\
...
```
You will get the URL, the browser will be started automatically, and the puzzle (website) will be hosted on your local server.
